...
import { bulkAssignPalletAction } from "@/lib/actions/clients";
...
export function BatchGrouper({ deliveries }: { deliveries: DeliveryListItem[] }) {
  ...
  const [state, action, pending] = useActionState(
    async (prev: { error?: string; success?: string }, formData: FormData) => {
      const res = await bulkAssignPalletAction(prev, formData);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <input
          ...
          placeholder="Buscar por número, cliente o lote actual…"
      ...
      <form action={action} className="panel sticky top-2 z-20 space-y-3 p-4 shadow-xl border-cat/40">
        ...
        {existingPallets.length > 0 && selectedIds.size > 0 ? (
          <div className="flex flex-wrap items-center gap-1.5 pt-1 text-xs text-muted">
            <span>Sugerencias de lotes activos:</span>
