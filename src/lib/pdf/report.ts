...
class ReportWriter {
  ...
  kv(label: string, value: string) {
    ...
    this.page.drawText(value, { x: 170, y: this.y, size: 10, font: this.font, color: BLACK });
    this.y -= 16;
  }
...
export async function buildDeliveryReportPdf(
  ...
  page.drawRectangle({ x: 0, y: 800, width: 595, height: 42, color: BLACK });
  page.drawRectangle({ x: 0, y: 800, width: 8, height: 42, color: YELLOW });
  ...
  const writer = new ReportWriter(doc, page, font, bold, 772);
  writer.text(`Entrega ${detail.number}`, { bold: true, size: 18 });
  writer.text("Informe de preparación / despacho — uso interno", { size: 9, color: MUTED });
  writer.gap(8);
    ...
    const mark = !req.applicable
      ...
      : req.status === "COMPLETE"
        ...
        : req.required
          ? "PEND"
          : "—";
    writer.text(`${mark}  ${req.label}${req.required ? " (obligatorio)" : ""}`, { size: 11 });
    const active = req.evidences.filter((ev) => !ev.voided_at);
    if (!req.applicable) {
      writer.text("No aplica", { size: 8, color: MUTED });
      continue;
    }
    if (active.length === 0) {
      writer.text("Sin evidencias", { size: 8, color: MUTED });
      continue;
    }
    for (const ev of active) {
      const caption = `${ev.filename} · ${ev.uploader_name ?? "—"} · ${formatDateTime(ev.created_at)}${ev.comment ? ` · ${ev.comment}` : ""}`;
  ...
  for (const event of detail.audit) {
    writer.text(
      `${formatDateTime(event.created_at)}  ${event.action}  ·  ${event.actor_name ?? "sistema"}`,
      { size: 8 },
    );
  }
  ...
  writer.gap(16);
  writer.text(
    "Marca: asset oficial Finning/CAT pendiente de autorización IT. Documento de uso interno.",
    { size: 7, color: MUTED },
  );
